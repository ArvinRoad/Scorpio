...
void AWeaponBaseServer::MultShootingEffect_Implementation() {
	...
	if(GetOwner() != UGameplayStatics::GetPlayerPawn(GetWorld(),0)) {
		...
		UGameplayStatics::SpawnEmitterAttached(MuzzleFlash,WeaponMesh,MuzzleFlashSocketName,FVector::ZeroVector,FRotator::ZeroRotator,FVector::OneVector,EAttachLocation::KeepRelativeOffset,true,EPSCPoolMethod::None,true);	// 枪口特效 EPSCPoolMethod::AutoRelease 表示用完后对池的操作(当前是自动返还到对现池) None|不存储对象池子
